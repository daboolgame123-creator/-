...
export const TransactionsList: React.FC<TransactionsListProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Quick Director Navigation & Read Filters Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-2.5 bg-stone-100/90 p-2.5 rounded-xl border border-stone-200">
        <div className="flex flex-wrap items-center gap-2">
          {/* Unread Only Toggle Button */}
          <button
            type="button"
            id="btn-filter-unread-only"
          ...
          >
            <span className={`w-2 h-2 rounded-full ${unreadOnly ? 'bg-white animate-ping' : 'bg-rose-500'}`}></span>
            <span>عرض الكتب غير المقروءة فقط 🔴</span>
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-extrabold ${
              unreadOnly ? 'bg-white text-rose-700' : 'bg-rose-100 text-rose-800'
            }`}>
              {totalUnreadCount}
            </span>
          </button>

          {/* Show All Books Button (if unread filter active) */}
          {unreadOnly && (
            <button
              type="button"
              onClick={() => setUnreadOnly(false)}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-white border border-stone-200 text-stone-700 hover:text-stone-900 text-xs font-semibold cursor-pointer"
            >
              عرض كافة الكتب ({transactions.length})
            </button>
          )}

          {/* Quick Mark All as Read Button */}
          {totalUnreadCount > 0 && onMarkAllAsRead && (
            <button
              type="button"
              id="btn-quick-mark-all-read"
              onClick={onMarkAllAsRead}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-stone-900 hover:bg-stone-800 text-amber-300 text-xs font-semibold transition-colors cursor-pointer shadow-xs"
              title="تحويل كل الكتب غير المقروءة إلى مقروءة بنقرة واحدة"
            >
        ...
        <div className="flex items-center gap-2 text-xs text-stone-600 mr-auto">
          <span className="hidden sm:inline text-stone-400">|</span>
          <span className="text-[11px] text-stone-500">
            {unreadOnly ? (
              <strong className="text-rose-700">تصفية: الكتب غير المقروءة فقط</strong>
            ) : (
              <span>المعروض: <strong>{filteredTransactions.length}</strong> من أصل <strong>{transactions.length}</strong></span>
            )}
          </span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-4 shadow-xs space-y-3.5">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pr-10 pl-14 py-2.5 rounded-lg border border-stone-200 text-xs sm:text-sm bg-stone-50/50 hover:bg-stone-50 focus:bg-white focus:ring-2 focus:ring-stone-800 focus:border-stone-800 outline-hidden transition-all shadow-2xs"
            />
            {searchTerm && (
              <button
                type="button"
                onClick={() => setSearchTerm('')}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-stone-400 hover:text-stone-800 bg-stone-200/60 hover:bg-stone-200 px-2 py-0.5 rounded cursor-pointer transition-colors"
              >
                مسح
              </button>
            )}
          ...
          {(searchTerm || unreadOnly || activeAdvancedCount > 0) && (
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg border border-stone-200 text-stone-600 hover:text-rose-700 hover:border-rose-200 hover:bg-rose-50 text-xs font-medium transition-colors cursor-pointer whitespace-nowrap"
        ...
        {isAdvancedOpen && (
          <div 
            ...
            className="p-4 rounded-xl bg-stone-50/90 border border-stone-200 space-y-3.5 transition-all animate-fadeIn"
          >
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-200/70 pb-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-stone-500 bg-stone-200/60 px-2 py-0.5 rounded-md">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className={`w-full py-2 px-2.5 rounded-lg border text-xs outline-hidden transition-all cursor-pointer ${
                    categoryFilter !== 'الكل'
                      ? 'border-amber-400 bg-amber-50/70 font-bold text-amber-900 shadow-2xs'
            ...
            <div className="pt-2 border-t border-stone-200/70 flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-stone-500 font-medium text-[11px]">حركة الكتاب:</span>
                ...
                {/* Status */}
                <div className="flex items-center gap-1.5">
                  <span className="text-stone-500 font-medium text-[11px]">حالة الإنجاز:</span>
                  <div className="inline-flex rounded-md border border-stone-200 bg-white p-0.5">
                ...
                {/* Priority */}
                <div className="flex items-center gap-1.5">
                  <span className="text-stone-500 font-medium text-[11px]">درجة الأسبقية:</span>
                  <div className="inline-flex rounded-md border border-stone-200 bg-white p-0.5">
              ...
              <button
                ...
                className="text-[11px] text-stone-600 hover:text-stone-900 font-medium px-2.5 py-1 rounded bg-stone-200/60 hover:bg-stone-200 transition-colors"
              >
                إخفاء لوحة الفلاتر
              </button>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-stone-100 text-xs">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-stone-400 font-medium text-[11px]">القسم:</span>
          ...
          <div className="flex flex-wrap items-center gap-2 mr-auto">
            ...
            <span className="text-[11px] text-stone-500 whitespace-nowrap">
              النتائج: <strong className="text-stone-800 font-bold">{filteredTransactions.length}</strong> معاملة
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
        ...
        <div className="inline-flex items-center gap-1 p-1 bg-stone-100/90 rounded-lg border border-stone-200 text-xs">
          <button
            type="button"
      ...
      {filteredTransactions.length === 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 overflow-hidden shadow-xs text-center py-12 px-4 space-y-3">
          ...
          {unreadOnly && (
            <button
              type="button"
              onClick={() => setUnreadOnly(false)}
              className="mt-2 text-xs font-semibold px-3 py-1.5 rounded-lg bg-stone-900 text-white"
            >
              عرض كافة المعاملات
            </button>
          )}
          ...
          {filteredTransactions.map((tr) => {
            ...
            return (
              <div
                ...
                className={`rounded-xl border transition-all cursor-pointer overflow-hidden p-4 sm:p-5 bg-white shadow-xs hover:shadow-md ${
                  isUnread
                    ? 'border-amber-400 bg-amber-50/20 border-r-6 border-r-rose-500 ring-1 ring-amber-300/30'
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-2.5 pb-3 border-b border-stone-100">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-mono font-bold px-2 py-0.5 rounded bg-stone-100 text-stone-600">
                      #{tr.sequence}
                    </span>
                    <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-md bg-stone-900 text-amber-300">
                      العدد: {tr.number}
                    </span>
                    <span
                    ...
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded bg-stone-200/80 text-stone-800">
                      {tr.subType}
                    </span>
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      {isUnread ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-300 shadow-2xs hover:bg-rose-200 transition-all cursor-pointer"
                        ...
                        <button
                          type="button"
                          onClick={(e) => onToggleReadStatus?.(tr.id, e)}
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-800 border border-emerald-200 hover:bg-emerald-100 transition-colors cursor-pointer"
                ...
                <div className="py-3.5 space-y-2.5">
                  <h4 className={`text-sm sm:text-base leading-relaxed ${
                    isUnread ? 'font-bold text-stone-950' : 'font-semibold text-stone-900'
                  }`}>
                    {tr.subject}
                  </h4>
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs pt-1">
                    <div className="flex items-center gap-1.5 text-stone-700">
                      ...
                      <span className="text-stone-400">الجهة المرتبطة:</span>
                      <span className="font-semibold text-stone-800">{tr.entity}</span>
                    </div>

                    {tr.employeeName && (
                      <div className="flex items-center gap-1.5 text-emerald-900 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200/80">
                        <User className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                        <span className="text-emerald-700 font-medium">المنتسب:</span>
                        <span className="font-bold">{tr.employeeName}</span>
                      </div>
                    )}

                    {tr.specificDetails?.purpose && (
                      <div className="text-stone-600 bg-stone-50 px-2 py-0.5 rounded border border-stone-200">
                        <span className="text-stone-400">الغرض: </span>
                        <span className="font-medium">{tr.specificDetails.purpose}</span>
                      </div>
                    )}
                    {tr.specificDetails?.destination && (
                      <div className="text-stone-600 bg-stone-50 px-2 py-0.5 rounded border border-stone-200">
                        <span className="text-stone-400">الوجهة: </span>
                        <span className="font-medium">{tr.specificDetails.destination}</span>
                      </div>
                    )}
                    {tr.specificDetails?.vehicle && (
                      <div className="text-stone-600 bg-stone-50 px-2 py-0.5 rounded border border-stone-200">
                        <span className="text-stone-400">العجلة: </span>
                        <span className="font-medium">{tr.specificDetails.vehicle}</span>
                      </div>
                    )}
                    {tr.specificDetails?.amount && (
                      <div className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 font-semibold">
                  ...
                  {tr.attachments && tr.attachments.length > 0 && (
                    <div className="pt-2 flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-1 text-stone-600 text-[11px] font-semibold bg-stone-100 px-2 py-1 rounded-md border border-stone-200">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {tr.attachments.slice(0, 5).map((att, i) => (
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full bg-stone-50 hover:bg-amber-100 text-stone-800 border border-stone-200 hover:border-amber-400 transition-all cursor-pointer group shadow-2xs"
                          ...
                          >
                            ...
                            <span className="truncate max-w-[130px] font-medium group-hover:text-amber-950">{att.name}</span>
                            <span className="text-amber-800 bg-amber-50/80 px-1.5 py-0.2 rounded text-[10px] font-bold">({att.type})</span>
                            <Eye className="w-3 h-3 text-stone-400 group-hover:text-amber-700 ml-0.5" />
                          </button>
                        ))}
                        {tr.attachments.length > 5 && (
                          <button
                            ...
                            className="text-[11px] font-bold text-stone-600 hover:text-amber-900 bg-stone-100 hover:bg-amber-100 px-2.5 py-1 rounded-full border border-stone-200 cursor-pointer"
                          >
                  ...
                  {tr.directorDirective && (
                    <div className="mt-2.5 p-3 rounded-xl bg-linear-to-r from-amber-50 to-orange-50/70 border border-amber-200/90 text-xs text-amber-950 flex items-start gap-2.5 shadow-2xs">
                      <span className="text-base leading-none shrink-0 mt-0.5">✍️</span>
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center justify-between gap-2 flex-wrap">
                          <span className="font-bold text-amber-900">هامش وتوجيه السيد المدير:</span>
                          {tr.directorDirective.date && (
                            <span className="text-[10px] text-amber-800/80 bg-amber-100/70 px-2 py-0.5 rounded font-semibold">
                              {tr.directorDirective.date}
                            </span>
                          )}
                        </div>
                        <p className="font-semibold leading-relaxed text-stone-900 bg-white/70 p-2 rounded-lg border border-amber-200/60">
                          «{tr.directorDirective.text}»
                        </p>
                        {tr.directorDirective.actionRequired && (
                          <span className="inline-block text-[10px] font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded-full border border-rose-200">
                  ...
                  {/* Director's Quick Directive Toolbar (When userRole === 'director') */}
                  {userRole === 'director' && (
                    <div 
                      className="mt-2.5 pt-2.5 border-t border-dashed border-stone-200 flex flex-col gap-2"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="flex flex-wrap items-center justify-between gap-1.5">
                        <span className="text-[11px] font-bold text-stone-700 flex items-center gap-1">
                          <span className="text-xs">✍️</span>
                          <span>هامش السيد المدير السريع:</span>
                        </span>
                        <div className="flex flex-wrap items-center gap-1">
                          <button
                            type="button"
                            onClick={() => handleQuickDirective(tr.id, 'يُحال للإجراء المطلوب وموافاتنا بالنتيجة', true)}
                            className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 transition-colors cursor-pointer shadow-2xs active:scale-95"
                            title="توجيه سريع: يُحال للإجراء"
                          >
                            + يُحال للإجراء
                          </button>
                          <button
                            type="button"
                            onClick={() => handleQuickDirective(tr.id, 'للحفظ والأرشفة أصولياً في أضبارة الموضوع', false)}
                            className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 transition-colors cursor-pointer shadow-2xs active:scale-95"
                            title="توجيه سريع: للحفظ والأرشفة"
                          >
                            + للحفظ والأرشفة
                          </button>
                          <button
                            type="button"
                            onClick={() => handleQuickDirective(tr.id, 'موافق أصولياً مع استكمال الإجراءات الإدارية', false)}
                            className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 transition-colors cursor-pointer shadow-2xs active:scale-95"
                            title="توجيه سريع: موافق أصولياً"
                          >
                            + موافق
                          </button>
                          <button
                            type="button"
                            onClick={() => handleQuickDirective(tr.id, 'يُعرض عليّ لاحقاً للمراجعة والمتابعة', false)}
                            className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 transition-colors cursor-pointer shadow-2xs active:scale-95"
                            title="توجيه سريع: يُعرض لاحقاً"
                          >
                            + يُعرض لاحقاً
                          </button>
                          <button
                            type="button"
                            onClick={() => {
                              if (activeDirectiveCardId === tr.id) {
                                setActiveDirectiveCardId(null);
                              } else {
                                setActiveDirectiveCardId(tr.id);
                                setCustomDirectiveText(tr.directorDirective?.text || '');
                                setDirectiveActionRequired(tr.directorDirective?.actionRequired || false);
                              }
                            }}
                            className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 transition-colors cursor-pointer shadow-2xs active:scale-95"
                            title="كتابة نص هامش مخصص"
                          >
                            ✏️ {tr.directorDirective ? 'تعديل الهامش' : 'هامش مخصص'}
                          </button>
                        </div>
                      </div>

                      {/* Expandable Custom Directive Input */}
                      {activeDirectiveCardId === tr.id && (
                        <div className="p-3 rounded-xl bg-stone-50 border-2 border-amber-300 space-y-2.5 shadow-inner">
                          <label className="block text-xs font-bold text-stone-800">
                            كتابة هامش أو توجيه المدير للمعاملة #{tr.sequence}:
                          </label>
                          <textarea
                            rows={2}
                            value={customDirectiveText}
                            onChange={(e) => setCustomDirectiveText(e.target.value)}
                            placeholder="اكتب توجيه أو ملاحظة السيد المدير هنا..."
                            className="w-full p-2.5 text-xs rounded-lg border border-stone-300 bg-white focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-hidden resize-none font-medium"
                            autoFocus
                          />
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <label className="flex items-center gap-1.5 text-xs font-semibold text-stone-700 cursor-pointer">
                              <input
                                type="checkbox"
                                checked={directiveActionRequired}
                                onChange={(e) => setDirectiveActionRequired(e.target.checked)}
                                className="rounded text-amber-600 focus:ring-amber-500"
                              />
                              <span>يتطلب إجراء فوري ومتابعة من شعبة الذاتية</span>
                            </label>
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => setActiveDirectiveCardId(null)}
                                className="text-xs px-3 py-1 rounded-lg text-stone-600 hover:bg-stone-200 font-medium"
                              >
                                إلغاء
                              </button>
                              <button
                                type="button"
                                onClick={() => handleSaveCustomDirective(tr.id)}
                                className="text-xs font-bold px-4 py-1.5 rounded-lg bg-amber-800 hover:bg-amber-900 text-white shadow-xs cursor-pointer active:scale-95"
                              >
                                تثبيت الهامش والتوجيه ✓
                              </button>
                            </div>
                          </div>
                        </div>
                ...
                <div className="border-t border-stone-100 pt-3 flex flex-wrap items-center justify-between gap-3 text-xs">
                  <div 
                  ...
                  >
                    ...
                    {tr.createdAt && (
                      <span className="text-stone-400 text-[11px] pr-2 border-r border-stone-200">
        ...
        <div className="bg-white rounded-xl border border-stone-200 overflow-hidden shadow-xs">
          <div className="overflow-x-auto">
            <table className="w-full text-right text-xs">
              <thead className="bg-stone-50/90 border-b border-stone-200 text-stone-600 font-semibold">
                <tr>
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors group cursor-pointer ${
                        isUnread 
                          ? 'bg-rose-50/30 hover:bg-rose-50/70 border-r-4 border-r-rose-500 font-medium' 
                    ...
                    >
                        ...
                        <div className="flex flex-col items-start gap-0.5">
                          {isUnread ? (
                            <button
                              ...
                              onClick={(e) => onToggleReadStatus?.(tr.id, e)}
                              title="كتاب جديد غير مقروء - انقر لتحديده كمقروء"
                              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-100 text-rose-800 border border-rose-300 shadow-2xs hover:bg-rose-200 transition-all cursor-pointer"
                            >
                              <span className="w-2 h-2 rounded-full bg-rose-600 animate-pulse"></span>
                              <span>غير مقروء 🔴</span>
                            </button>
                            ...
                            <button
                              ...
                              onClick={(e) => onToggleReadStatus?.(tr.id, e)}
                              title="تم الاطلاع - انقر للتبديل"
                              className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-emerald-50 text-emerald-800 border border-emerald-200 hover:bg-emerald-100 transition-colors cursor-pointer"
                            >
                      ...
                      <td className="py-3.5 px-4 max-w-xs sm:max-w-md">
                        <p className={`line-clamp-2 leading-relaxed ${isUnread ? 'text-stone-900 font-bold' : 'text-stone-800 font-medium'}`}>
                          {tr.subject}
                        </p>
                        {tr.directorDirective && (
                          <div className="mt-1 text-[11px] text-amber-900 bg-amber-50 px-2 py-0.5 rounded border border-amber-200 inline-flex items-center gap-1">
                            <span>✍️</span>
                      ...
                      <td className="py-3.5 px-4 text-center whitespace-nowrap">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-stone-100 hover:bg-amber-100 text-stone-700 hover:text-amber-900 text-[11px] font-medium transition-colors cursor-pointer border border-stone-200"
